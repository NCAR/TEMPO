...
! Densities of rain, snow, graupel, and cloud ice.
    real(wp), parameter :: rho_w2 = 1000.0 ! AAJ change to rho_w2 to solve MPAS same var name conflict
    real(wp) :: rho_s2 = 100.0 ! AAJ change to rho_s2 to solve MPAS same var name conflict
    ! real(wp), parameter :: rho_g = 500.0
...
! Water vapor and air gas constants at constant pressure
    ...
    real(wp), parameter :: oRv = 1./Rv
    real(wp), parameter :: R = 287.04
    real(wp), parameter :: RoverRv = 0.622
    real(wp), parameter :: Cp2 = 1004.0 ! AAJ change to Cp2 to solve MPAS same var name conflict
...
! AAJ Not needed 
    ...
    real(dp), allocatable, dimension(:,:,:) :: tpc_wev, tnc_wev
